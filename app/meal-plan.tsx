    ...
    <View style={s.page}>
      <View style={s.hero}>
        ...
        <Pressable onPress={() => setShowShopping((visible) => !visible)} style={s.shoppingToggle} accessibilityRole="button" accessibilityLabel={showShopping ? 'Show recipes' : 'Show shopping list'}>
          <Text style={s.shoppingToggleText}>{showShopping ? 'View recipes' : 'View shopping list'}</Text><Text style={s.shoppingToggleArrow}>{showShopping ? '⌃' : '⌄'}</Text>
        </Pressable>
      </View>
      ...
      {showShopping ? <ShoppingList items={shopping.items} total={shopping.total} onRemove={removeFromShoppingList} refreshing={refreshing} /> : <View style={s.plannerCard}>
        ...
        <View style={s.navigation}>
          <Pressable disabled={index === 0} onPress={() => selectIndex(index - 1)} style={[s.navButton, index === 0 && s.navDisabled]} accessibilityRole="button" accessibilityLabel="Previous day"><Text style={[s.navArrow, index === 0 && s.navDisabledText]}>‹</Text><Text style={[s.navLabel, index === 0 && s.navDisabledText]}>Previous</Text></Pressable>
          <Pressable disabled={index === DAYS.length - 1} onPress={() => selectIndex(index + 1)} style={[s.navButton, s.nextButton, index === DAYS.length - 1 && s.navDisabled]} accessibilityRole="button" accessibilityLabel="Next day"><Text style={[s.navLabel, s.nextLabel, index === DAYS.length - 1 && s.navDisabledText]}>Next</Text><Text style={[s.navArrow, s.nextLabel, index === DAYS.length - 1 && s.navDisabledText]}>›</Text></Pressable>
        </View>
      </View>}
    </View>
  </Screen>;
